...
def get_y_json(uri):
    response_xml = y_session.session.get(uri)
    return xmltodict.parse(response_xml.content, dict_constructor=dict) 

fantasy_weeks = y.load_json('yahoo_fantasy_api/data/' + league_code + '/fantasy_weeks.json')
today = dt.datetime.today()
date = convert_date(today)  
week = search_fantasy_week(date)['week']
#date = '2018-10-28'  # use sunday dates to get previous week
#week = 2

# get winners for real team, run this every day - to do: check for current and past
#week = search_fantasy_week(date)['week'] - 1
scoreboard = get_scoreboard(y, league_code, week)
...
daily_stats = get_daily_stats(date, team_type)
...
df_actives = check_active_players(df)
...
matchups = get_matchups(week)  # get subset of data from real matchups
auto_scoreboard = get_auto_matchup(matchups, week, df)  # from real matchups create auto team matchup
...
t_auto_winners = get_real_auto_winners(top_auto_scoreboard) # create stat winners dict
b_auto_winners = get_real_auto_winners(bottom_auto_scoreboard)

t_auto_winners = add_winner_team_key(t_auto_winners) # add winner team key
b_auto_winners = add_winner_team_key(b_auto_winners)
