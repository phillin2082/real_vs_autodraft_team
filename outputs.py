...
for i in reversed(range(1, week+1)):
    ...
    header_values = get_header_values(real_winners[0])
    blank_values = get_blank_values(real_winners[0])
    ...
    for index, (r, ta, ba) in enumerate(zip(real_winners, top_auto_winners, bottom_auto_winners)):    
        ...
        r_all = organize_matchup_data(r, 'real')
        ta_all = organize_matchup_data(ta, 'ta')
        ba_all = organize_matchup_data(ba, 'ba')
        for  x in r_all:
        ...
        match_output.append(blank_values)                        
        match_output.append(blank_values)
        if index != (len(bottom_auto_winners) - 1):
